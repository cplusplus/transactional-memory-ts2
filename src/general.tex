...
\pnum
Unless otherwise specified, these wording changes applies P2066R10 relative to C++ Working Draft, N4849.
...
\begin{itemize}
    ...
    \item are declared in a namespace whose name appends \tcode{::experimental::transactional\_memory\_v2} to a namespace defined in the C++ Standard Library, such as \tcode{std}, or
...
\tcode{__cpp_lib_transactional\_memory\_v2}  & \tcode{\tsver}  & \tcode{<experimental/transactional\_memory\_v2>} \\
\end{floattable}
...
contents in \tcode{std::experimental::transactional\_memory\_v3}, \tcode{std::experimental::transactional\_memory\_v4}, etc., with the most recent
...
will be moved into the next version of the C++ Standard by removing the \tcode{experimental::transactional\_memory\_v$N$} segment of its
