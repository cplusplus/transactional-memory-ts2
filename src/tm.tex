...
%%5.10
The requirements of \CppXref{lex.name} apply to this document. 

In \CppXref{lex.name}, in addition to the requirements of Table 4, this document requires the following in Table \cxxref{tab:lex.name.special} - Identifiers with special meaning: 
%%add %\added
\begin{quote}
{\tcode{atomic}}
\end{quote}
...
\rSec1[intro.execution]{Sequential execution}
%%6.9.1
The requirements of \CppXref{intro.execution} apply to this document. 

In \CppXref{intro.execution}, in addition to the requirements of paragraph 5,  this document requires the following in paragraph 5:
...
\rSec1[intro.races]{Data races} 
%%6.9.2.2
The requirements of \CppXref{intro.races} apply to this document.

In \CppXref{intro.races}, instead of the requirements of  paragraph 6,  this document requires the following in paragraph 6:
...
In \CppXref{intro.races}, in addition to the requirements of paragraph 20,  this document requires the following after paragraph 20:
...
In \CppXref{intro.races}, instead of the requirements of paragraph 21,  this document requires the following in paragraph 21:
...
In \CppXref{intro.races}, in addition to the requirements of paragraph 21,  this document requires the following after paragraph 21:
\begin{quote}
%%\setcounter{Paras}{21}
%%\pnum
\begin{note}
...
\rSec1[intro.progress]{Forward progress} 
%%6.9.2.3
The requirements of \CppXref{intro.progress} apply to this document. 

In \CppXref{intro.progress}, instead of the requirements of paragraph 1,  this document requires the following in paragraph 1:
...
%%8.1
%%\pnum
The requirements of \CppXref{stmt.pre} apply to this document. 

In \CppXref{stmt.pre}, instead of the requirements of paragraph 1,  this document requires the following in paragraph 1 in the grammar production:
...
\rSec1[stmt]{Statement}

%%8 and 8.8
The requirements of \CppXref{stmt} apply to this document.

In \CppXref{stmt}, in addition to the requirements of \CppXref{stmt.dcl},  this document requires the following before \CppXref{stmt.dcl}:

\begin{quote}
\rSec2[stmt.tx]{Atomic statement}  
...
%%15.11
The requirements of \CppXref{cpp.predefined} apply to this document. 

In \CppXref{cpp.predefined}, in addition to the requirements of Table 19, this document requires the following in Table \cxxref{tab:cpp.predefined.ft} - Feature-test Macros: 
...
\rSec1[conforming]{Conforming implementations}
%%16.4.6
%%\setcounter{subsection}{6}
%%\setcounter{subsubsection}{16}

The requirements of \CppXref{conforming} apply to this document.

In \CppXref{conforming}, in addition to the requirements of \CppXref{lib.types.movedfrom},  this document requires the following after \CppXref{lib.types.movedfrom}:


%%Add a new subclause after \CppXref{lib.types.movedfrom}:

\begin{quote}
\rSec2[atomic.use]{Functions usable in an atomic block}
